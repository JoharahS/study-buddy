...
class Test(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        container.configure(bg="grey", bd=20)
        container.pack(side="top", fill="both", expand=True)

        frame = Calculator(container, self)
        frame.pack()
...
class Calculator(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        def button_click(char):
            ...
            text_input.set(self.calc_operator)
        ...
        def fact_func():
            result = str(factorial(int(self.calc_operator)))
        ...
        tk_calc = Frame(self, bg="grey")
        tk_calc.pack()

        self.calc_operator = ""
        text_input = StringVar()

        text_display = Entry(tk_calc, font=('sans-serif', 15, 'bold'), textvariable=text_input,
                             bd=5, insertwidth=5, bg='white', justify='right').grid(columnspan=5, padx=10, pady=15)

        button_params = {'bd': 5, 'fg': 'black', 'bg': 'white', 'font': ('calibri', 15, 'bold')}
        button_params_main = {'bd': 5, 'fg': 'black', 'bg': 'grey', 'font': ('calibri', 15, 'bold')}
        ...
        delete_one = Button(tk_calc, bd=5, fg='#000', font=('calibri', 15, 'bold'),
                            text='DEL', command=button_delete, bg='grey').grid(row=6, column=3, sticky="nsew")
        delete_all = Button(tk_calc, bd=5, fg='#000', font=('calibri', 15, 'bold'),
                            text='AC', command=button_clear_all, bg='grey').grid(row=6, column=4, sticky="nsew")
